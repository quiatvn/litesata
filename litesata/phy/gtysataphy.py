...
class GTYLiteSATAPHYCRG(Module):
    def __init__(self, refclk, pads, gty, gen):
        ...
        tx_mmcm_clkout_freq = {
            "gen1":  75e6/(gty.data_width/16),
            "gen2": 150e6/(gty.data_width/16),
            "gen3": 300e6/(gty.data_width/16),
        }
        #self.submodules += tx_mmcm
        #self.specials += Instance("BUFG", i_I=gty.txoutclk, o_O=tx_mmcm_clkin)
        ...
        self.specials += [
            #AsyncResetSynchronizer(self.cd_sata_tx, ~(gtp.cplllock & tx_mmcm.locked) | self.tx_reset),
...
class GTYLiteSATAPHY(Module):
    def __init__(self, pads, gen, clk_freq, data_width=16):
        ...
        self.gty_params = dict(
            ...
            p_CH_HSPMUX                      = 0b0100000001000000,
        ...
        self.gty_params.update(
            ...
            p_RXCDR_CFG2                     = 0b0000001000111001 if gen == "gen1" else 
                                               0b0000001001001001 if gen == "gen2" else
                                               0b0000001001011001,
            p_RXCDR_CFG2_GEN2                = 0b1000111001 if gen == "gen1" else 
                                               0b1001001001 if gen == "gen2" else
                                               0b1001011001,
            p_RXCDR_CFG2_GEN3                = 0b0000001000111001 if gen == "gen1" else
                                               0b0000001001001001 if gen == "gen2" else
                                               0b0000001001011001,
            ...
            p_RXPI_CFG0                      = 0b0000000100000010,
            p_RXPI_CFG1                      = 0b0000000001010100,
            p_RXPMACLK_SEL                   = "DATA",
        ...
        self.gty_params.update(
            ...
            p_TXPH_CFG                       = 0b0000001100100011 if gen == "gen3" else
                                               0b0000011100100011,
            p_TXPH_CFG2                      = 0b0000000000000000,
            p_TXPH_MONITOR_SEL               = 0b00000,
            p_TXPI_CFG0                      = 0b0000000100000000,
        ...
        # Monitor frequency ------------------------------------------------------------------------        
        self.txclk_period = txclk_period = Signal(29)
        self.specials += [
            Instance("clkcnt",
                p_CLKCOUNT = clk_freq,
                i_clkref   = ClockSignal("sys"),
                i_clkmon   = ClockSignal("sata_tx"),
                i_rst      = 0b0,
                o_oval     = txclk_period
                )
            ]

        self.rxclk_period = rxclk_period = Signal(29)
        self.specials += [
            Instance("clkcnt",
                p_CLKCOUNT = clk_freq,
                i_clkref   = ClockSignal("sys"),
                i_clkmon   = ClockSignal("sata_rx"),
                i_rst      = 0b0,
                o_oval     = rxclk_period
                )
            ]
        ...
        # DEBUG Signals ----------------------------------------------------------------------------        
        self.txcominit_db = Signal()
        self.txcominit_db = txcominit
        self.txcomfinish_db = Signal()
        self.txcomfinish_db = txcomfinish

        self.rxcominit_db = Signal()
        self.rxcominit_db = rxcominitdet

        self.txcomwake_db = Signal()
        self.txcomwake_db = txcomwake


        self.rxcomwake_db = Signal()
        self.rxcomwake_db = rxcomwakedet

        self.waitcnt = Signal(28)
        self.sync.sata_tx += [
            If(txcominit | txcomwake, self.waitcnt.eq(1)
            ).Elif(txcomfinish, self.waitcnt.eq(0)
            ).Elif(self.waitcnt > 0, self.waitcnt.eq(self.waitcnt+1)
            )
        ]
